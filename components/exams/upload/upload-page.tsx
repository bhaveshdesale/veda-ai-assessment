...
export function UploadPage({
  ...
  const [questionPaperError, setQuestionPaperError] =
    useState<UploadError | null>(null);

  const [answerSheet, setAnswerSheet] =
    useState<UploadFile | null>(null);

  const [answerSheetError, setAnswerSheetError] =
    useState<UploadError | null>(null);
    ...
    <section className="flex min-h-[calc(100vh-64px)] flex-col items-center px-5 py-10 sm:py-14">
      <div className="text-center">
        ...
        <p className="mx-auto mt-2 max-w-[430px] text-[10px] leading-5 text-[#77746c] sm:text-[11px]">
          Upload the question paper and one student answer
          sheet to extract, map and review the assessment.
        </p>
      </div>
      ...
      <div className="mt-8 grid w-full max-w-[680px] gap-4 sm:grid-cols-2">
        <UploadCard
          ...
          onFileChange={updateQuestionPaper}
        />
        ...
        <UploadCard
          ...
          onFileChange={updateAnswerSheet}
        />
      </div>
